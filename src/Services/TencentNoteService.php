...
class TencentNoteService implements NoteInterface
{
    private $config = [];
    ...
    public function sendNote($phone, $code)
    {
        try {
            ...
            $cred = new Credential($this->config['secret_id'], $this->config['secret_key']);//写入secret_id和secret_key
            ...
            $req->SmsSdkAppId = $this->config['sms_sdk_app_id'];
            /* 短信签名内容: 使用 UTF-8 编码，必须填写已审核通过的签名，签名信息可登录 [短信控制台] 查看 */
            $req->SignName = $this->config['sign_name'];
            ...
            $req->TemplateId = $this->config['template_id'];
            /* 模板参数: 若无模板参数，则设置为空*/
